...
class Slider < UIControl
  ...
  def update
    ...
    if left.contains(mousepos[0],mousepos[1])
      if Input.repeatcount(0x01)>100
        self.curvalue-=10
        self.curvalue=self.curvalue.floor
      elsif Input.repeatcount(0x01)>50
        self.curvalue-=5
        self.curvalue=self.curvalue.floor
      else
        self.curvalue-=1
...
class AnimationCanvas < Sprite
  ...
  def play
    if !@playing
      ...
      @sprites["pokemon1"].bitmap=@target
      @sprites["pokemon1"].z=16
      pbSpriteSetAnimFrame(@sprites["pokemon0"],
      ...
      pbSpriteSetAnimFrame(@sprites["pokemon1"],
      ...
      @sprites["pokemon0"].y+=BORDERSIZE
      @sprites["pokemon1"].x+=BORDERSIZE
      @sprites["pokemon1"].y+=BORDERSIZE
      prevoldstate=[]
      oldstate=[]
      for i in 0...PBAnimation::MAXSPRITES
  ...
  def setPreviousFrame(i)
    if @currentframe>0
      ...
      if cel!=nil
        @lastframesprites[i].ox=64
        @lastframesprites[i].oy=64
        @lastframesprites[i].selected=false
  ...
  def updateInput
    ...
    if cel
      if Input.repeatex?(0x2E) && self.deletable?(@currentcel) # "Delete"
      ...
      if Input.repeatex?(0x52) # "R" for rotate left
