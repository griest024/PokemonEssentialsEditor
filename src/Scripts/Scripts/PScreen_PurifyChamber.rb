...
# Purify Chamber treats Normal/Normal matchup as super effective
  def self.typeAdvantage(p1,p2)
    if isConst?(p1,PBTypes,:NORMAL) && isConst?(p2,PBTypes,:NORMAL)
      return true
    end
    return PBTypes.isSuperEffective?(p1,p2)
  end

  def self.isSuperEffective(p1,p2)
    if typeAdvantage(p1.type1,p2.type1) || typeAdvantage(p1.type1,p2.type2)
      return true
    end
    return false
  end
...
class PurifyChamber # German: der Kryptorbis
  ...
  def initialize
    ...
    @currentSet=0
    NUMSETS.times {|i| 
       @sets[i]=PurifyChamberSet.new
    }
  end

  def currentSet=(value)
    if value>=0 && value<NUMSETS
      @currentSet=value
    end
  end
    ...
    for set in 0...NUMSETS
      ...
      if @sets[set].shadow
        if @sets[set].shadow.heartgauge>0
          ...
          if isPurifiable?(set)
            Kernel.pbMessage(_INTL("Your {1} in the Purify Chamber is ready for purification!",@sets[set].shadow.name))
          end
        end
      end
    end
        ...
        if curpkmn || heldpkmn
          ...
          if choice==0
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.isShadow?)!=(curpkmn.isShadow?)
...
class PurifyChamberSetView < SpriteWrapper
  ...
  def checkCursor(index)
    if @cursor==index
      @view.x=@__sprites[index].x
      @view.y=@__sprites[index].y
      @view.visible=true
    end
  end
  ...
  def refresh
    ...
    @__sprites[1+PurifyChamber::SETSIZE*2].x=@view.x
    @__sprites[1+PurifyChamber::SETSIZE*2].y=@view.y-6
