...
class PKMNEE::Control::TileView < JavaFX::StackPane
	...
	attr_accessor :tile, :image, :isSelected, :position
	...
	def select(id = nil)
		getChildren.setAll(@image_view, JavaFX::ImageView.new(resource_url(:images, "select-#{@@select_images[id]}.png").to_s)) if id
		@isSelected = true
		self
	end
